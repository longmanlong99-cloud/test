...
【铁律修复】
1. NYMO & MCO 指标逻辑 100% 从电脑版 (21 factor 2026-01-12A.py) 移植，无任何修改。
2. 彻底清洗 NYMO URL，移除导致报错的 Markdown 链接标记。
3. 严格冻结 A4.py 的其余功能模块，确保系统核心链路完全恢复。
"""
...
# ==============================================================================
# 【爬虫层】WebScraper (Iron Rule Stable)
# ==============================================================================
class WebScraper:
    def __init__(self):
        ...
        try:
            ...
            if r.status_code==200:
                ...
                if scr and GENAI_API_KEY:
                    ...
                    prompt = """Analyze image. 1. Extract NYSE data (for Hindenburg): adv, dec, unch, high, low, adv_vol, dec_vol (Composite). 2. Extract NASDAQ data (for Breadth): 'nasdaq_adv', 'nasdaq_dec', 'nasdaq_unch'. Return ONE JSON object."""
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if scr_url:
                    ...
                    prompt = """Analyze StockCharts image for "$NYMO". Locate data legend top left. Extract value labeled "Last", "Close", or final number. Return JSON: {"value": -12.34}"""
...
class CrashWarningSystem:
    ...
    def fetch_and_calculate(self):
        ...
        fg, _ = self.scraper.fetch_fear_greed()
        margin_yoy, _ = self.scraper.fetch_margin_debt()
        lei_d, _ = self.scraper.fetch_lei()
        ...
        if wsj:
            ...
            m_val = mco if mco else (wsj['adv']-wsj['dec'])
            h_stat = 2 if (spx_trend_up and h_pct>2.2 and l_pct>2.2 and m_val<0) else (1 if (h_pct>2.2 and l_pct>2.2) else 0)
            h_ctx = f"SPX趋势:{'多' if spx_trend_up else '震'}\n新高:{h_pct:.1f}% | 新低:{l_pct:.1f}%\nMCO:{m_val:.1f}"
        indicators.append(["Hindenburg Omen (凶兆)", h_stat, h_ctx, "触发: 趋势+双边+MCO<0"])

        # NYMO
        n_stat = 0; n_txt = "N/A"
        if nymo is not None:
            n_stat = 2 if abs(nymo)>60 else 0
            n_txt = f"读数:{nymo:.1f}"
        indicators.append(["StockCharts 广度 ($NYMO)", n_stat, n_txt, "标准: >60 或 <-60 触发"])

        # NASDAQ Breadth
        if wsj:
            ratio = round(wsj['nasdaq_adv']/wsj['nasdaq_dec'], 2) if wsj.get('nasdaq_dec') else 0
            indicators.append(["NASDAQ 广度 (A/D Ratio)", 1 if ratio<1.0 else 0, f"Ratio: {ratio}", "标准: < 1.0 跌多涨少"])
    ...
    def generate_chart(self):
        ...
        ax.text(0.5, 0.96, "美股崩盘预警系统 Pro V10.104 (Restoration)", ha='center', fontweight='bold', fontsize=38, color=self.colors['title'])
