                    ...
                    try:
                        ...
                        if ai_resp.text:
                            ...
                            if match:
                                ...
                                if val is not None:
                                    nymo_val = float(val)
                                    p_ok(f"StockCharts ($NYMO) 视觉提取成功: {nymo_val}")
                                    return nymo_val
                    except Exception as e:
                        p_err(f"AI 视觉识别失败: {e}")
            else:
                p_err(f"Firecrawl 请求失败: {resp.status_code}")

        except Exception as e:
            p_err(f"NYMO 抓取流程异常: {e}")
            
        return nymo_val

    # --- 8.6 [升级] MCO 双重抓取 (100% 移植电脑版逻辑) ---
...
class CrashWarningSystem:
    ...
    def fetch_and_calculate(self):
        ...
        try:
            ...
            closes = []
            # 严格复刻老代码的分批下载逻辑 (80只一批)
            for i in range(0, len(tickers), 80):
                batch = tickers[i:i+80]
                data = yf.download(batch, period="2y", auto_adjust=True, progress=False)
                # 兼容 MultiIndex 提取
                if isinstance(data.columns, pd.MultiIndex):
                    try: closes.append(data['Close'])
                    except: closes.append(data)
                else:
                    closes.append(data)
            
            full_data = pd.concat(closes, axis=1).dropna(axis=1, how='all').ffill()
        ...
        if real_nymo is not None:
            if real_nymo > 60 or real_nymo < -60: st=2
            # 100% 复刻 NYMO 格式
            desc_nymo = "中性区 (正常波动)"
            if real_nymo > 60: desc_nymo = "历史高峰区 (极度超买)"
            elif real_nymo < -60: desc_nymo = "历史低谷区 (极度超卖)"
            txt = f"读数: {real_nymo:.2f}\n【定性】 {desc_nymo}"
        ...
        try:
            r = rsp/spy
            curr = r.iloc[-1]; ma = r.rolling(50).mean().iloc[-1]
...
def run_smt_log():
    ...
    if 'NQ=F' in df and 'ES=F' in df:
        ...
        if nq and not es: p_txt("📊 [10日 期货SMT]: 🔴 [看跌] 科技拉升，标普不跟\n   解读: 资金只敢做多高流动性的纳指，不敢全面做多，是诱多信号。")
    ...
    if 'SPY' in df:
        ...
        if abs((curr-ma20)/ma20)<0.006: p_txt("   🔥 [信号]: 完美回踩 MA20")
        else: 
            p_txt("   🚧 [信号]: 逼近前高阻力")
