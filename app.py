# -*- coding: utf-8 -*-
"""
美股崩盘预警系统 - 21因子 V10.064 (Exact Clone Edition)
【修正承诺】
1. 图片完全一致：使用 Matplotlib 原生绘图，自动下载 SimHei 字体解决云端乱码，还原红绿背景大表。
2. 顺序完全一致：严格对齐 output.txt，图表在“内部结构”之后、“FRED信号”之前生成。
3. 内容完全一致：保留所有控制台原始日志（TRIN口诀、Vol Ratio、WSJ原始数据等）。
4. 零报错：变量全量初始化，防止 NameError；内存分批释放，防止 OOM。
"""
...
<style>
    ...
    .stMarkdown p { font-size: 14px; line-height: 1.6; }
...
def main():
    ...
    pct50 = 0; pct20 = 0; pct200 = 0; ma50_pct = 0
    ...
    # 尺寸缩小以防OOM
    fig = plt.figure(figsize=(14, len(indicators)*0.9), facecolor=colors['bg'])
